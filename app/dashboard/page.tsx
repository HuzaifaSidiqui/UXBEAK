...
import { Settings, RefreshCw, Check, X, Eye, EyeOff, AlertTriangle, AlertCircle, Info, Lightbulb } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const [verifyStatus, setVerifyStatus] = useState<{ isTracking: boolean; recentEvents: number; lastEvent: string | null; message?: string } | null>(null);
  const [showEventProperties, setShowEventProperties] = useState<{ [key: number]: boolean }>({});
  ...
  // Redirect anonymous users to signin
  useEffect(() => {
    if (!authLoading && !user) {
  ...
  // fetch recent events and trackingCode - FIXED data structure
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      if (eventsData.length === 0) {
        toast.success("No events found yet - events will appear after you install the tracking code");
      } else {
  ...
  // fetch sessions (from /api/session)
  const fetchSessions = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const txt = await res.text();
        throw new Error(txt || "Failed to fetch sessions");
      }

      const data = await res.json();
      // data.sessions expected or fallback
  ...
  // Process AI response to ensure proper card format
  // In your dashboard component, update the processAIResponse function:
const processAIResponse = useCallback((data: any): Insight[] => {
  console.log("🔄 Processing AI response:", data);
  
  // If data is already a properly formatted insights array
  if (Array.isArray(data) && data.length > 0 && data[0].title && data[0].description) {
    return data;
  }

  // If data has insights property
  ...
  if (typeof data === 'string') {
    try {
      ...
      return processAIResponse(parsed); // Recursive call with parsed data
    } catch {
      // If it's just a plain string, create a single insight
      return [{
  ...
  return [{
    ...
    severity: "info",
    category: "general", 
    recommendation: "Review the insights above for specific recommendations",
    impact: "Improved user experience",
    confidence: 100
  }];
}, []);

  // run AI analysis using /api/analyze
  const runAnalysis = useCallback(async () => {
    ...
    if (events.length === 0 && sessions.length === 0) {
      toast.error("No data available for analysis. Please wait for events to be captured.");
      return;
    }
    ...
    try {
      ...
      // prepare payload: sessionData from sessions or events
      const sessionData = {
        recentEvents: events.slice(0, 200),
        sessions: sessions.slice(0, 200),
      };

      // minimal pageData derived from events (you can extend)
      const pageData = {
        pageUrl: events?.[0]?.properties?.$current_url || "/",
        totalClicks: events.reduce((acc, e) => acc + (e.properties?.clicks_count || 0), 0),
        heatmapSummary: events.slice(0, 50).map((e) => ({ event: e.event, props: e.properties })),
      };
      ...
      // Process the AI response to ensure proper card format
      const processedInsights = processAIResponse(data);
      setInsights(processedInsights);

      // Toast success
      toast.success(`Analysis complete - found ${processedInsights.length} issues`);
    } catch (err: any) {
      ...
      setInsights([
        {
          ...
          description: "Users are experiencing difficulty finding the checkout button, leading to cart abandonment.",
          severity: "high",
          category: "conversion",
          recommendation: "Increase button contrast and size, add micro-animations on hover",
        ...
        },
        {
          ...
          description: "Form fields are misaligned on mobile devices causing user frustration during signup.",
          severity: "medium",
          category: "responsive",
          recommendation: "Implement CSS grid for form layout and test on multiple screen sizes",
        ...
        },
        {
          ...
          description: "Product images are taking too long to load, affecting user engagement.",
          severity: "medium",
          category: "performance",
          recommendation: "Implement lazy loading and compress images below 100KB",
  ...
  // copy snippet helper - fetches the exact tracking script from the API
const copySnippet = useCallback(async () => {
  ...
  try {
    ...
    // Fetch the exact tracking script from your API
    ...
    // Fallback: generate basic snippet manually
  ...
  // Toggle event properties visibility
  ...
  const getSeverityConfig = (severity: string) => {
    switch (severity) {
      case 'high':
        return { icon: AlertTriangle, color: 'text-red-400', bgColor: 'bg-red-500/10', borderColor: 'border-red-500/20' };
      case 'medium':
        return { icon: AlertCircle, color: 'text-yellow-400', bgColor: 'bg-yellow-500/10', borderColor: 'border-yellow-500/20' };
      case 'low':
        return { icon: Info, color: 'text-blue-400', bgColor: 'bg-blue-500/10', borderColor: 'border-blue-500/20' };
      default:
        return { icon: Lightbulb, color: 'text-green-400', bgColor: 'bg-green-500/10', borderColor: 'border-green-500/20' };
    }
  };
  ...
  const renderInsightCard = (insight: Insight, index: number) => {
    const { icon: SeverityIcon, color, bgColor, borderColor } = getSeverityConfig(insight.severity || 'info');
    
    return (
      <Card key={index} className={`${bgColor} ${borderColor} border-2 backdrop-blur-sm transition-all hover:scale-[1.02] hover:shadow-xl`}>
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex flex-col items-end space-y-2 ml-4">
              <Badge 
                variant="outline" 
                className={`
                  ${insight.severity === 'high' ? 'bg-red-500/20 border-red-500/50 text-red-300' :
                    insight.severity === 'medium' ? 'bg-yellow-500/20 border-yellow-500/50 text-yellow-300' :
                    insight.severity === 'low' ? 'bg-blue-500/20 border-blue-500/50 text-blue-300' :
          ...
          <div className="flex flex-wrap gap-4 mb-4 text-sm">
            {insight.event_type && (
              <div className="flex items-center space-x-1">
                ...
                <span className="text-white font-medium">{insight.event_type}</span>
              </div>
            )}
            {insight.element && (
              <div className="flex items-center space-x-1">
                <span className="text-white/60">Element:</span>
                <span className="text-white font-medium">{insight.element}</span>
              </div>
            )}
            {insight.category && (
              <div className="flex items-center space-x-1">
          ...
          {insight.recommendation && (
            <div className="bg-black/20 rounded-lg p-4 border border-white/10">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="text-white font-semibold text-sm">Recommendation</span>
              </div>
    ...
    <Card className="bg-black/40 backdrop-blur-subtle border-white/10">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-lg text-white font-semibold">Tracking Status</CardTitle>
            <CardDescription className="text-white/60">Is your tracking code sending events to PostHog?</CardDescription>
          </div>
          <div className="flex items-center gap-2">
            <Button size="sm" onClick={verifyTracking} disabled={isRunningVerify} variant="outline">
              {isRunningVerify ? <RefreshCw className="h-4 w-4 animate-spin" /> : "Verify"}
            </Button>
            <Button size="sm" onClick={copySnippet} variant="ghost" className="text-white/70">
              Copy Snippet
            </Button>
          </div>
        </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              ...
              <p className="text-xs text-white/60 mt-2">
                ...
                {" · "}Last event: <strong>{verifyStatus.lastEvent ? new Date(verifyStatus.lastEvent).toLocaleString() : "N/A"}</strong>
              </p>
            </div>
            <div className="w-12 h-12 flex items-center justify-center rounded-full bg-white/5">
              {verifyStatus.isTracking ? <Check className="text-green-400" /> : <X className="text-red-400" />}
            </div>
          </div>
        ) : (
          <p className="text-white/60">No verification performed yet. Click Verify to check PostHog.</p>
        )}
      </CardContent>
    </Card>
  );
  ...
  const renderEventsCard = () => (
    <Card className="bg-black/30 backdrop-blur border-white/10">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-lg text-white font-semibold">Recent Events</CardTitle>
            <CardDescription className="text-white/60">Latest events captured for your tracking code</CardDescription>
          </div>
          <Button 
            size="sm" 
            onClick={fetchEvents} 
            ...
            variant="outline"
          >
            {loadingEvents ? <RefreshCw className="h-4 w-4 animate-spin" /> : "Refresh"}
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        {loadingEvents ? (
          ...
          <ul className="space-y-3">
            {events.map((event, index) => (
              <li key={event.id || index} className="p-3 bg-white/5 border border-white/6 rounded-lg text-sm">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="font-medium text-white mb-1">{event.event || "Unknown Event"}</div>
                  ...
                  >
                    {showEventProperties[index] ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                  </Button>
                </div>
              </li>
            ))}
          ...
          <div className="text-center py-8">
            ...
            <p className="text-xs text-white/40">
              Install the tracking snippet on your website to start capturing events
            </p>
          </div>
        )}
      </CardContent>
  ...
  const renderSessionsCard = () => (
    <Card className="bg-black/30 backdrop-blur border-white/10">
      <CardHeader>
        <CardTitle className="text-lg text-white font-semibold">Stored Sessions</CardTitle>
        <CardDescription className="text-white/60">Sessions your app recorded (for AI analysis)</CardDescription>
      </CardHeader>
      <CardContent>
        {loadingSessions ? (
          <div className="flex items-center justify-center py-8">
          ...
          <ul className="space-y-2">
            {sessions.map((s: any, idx: number) => (
              <li key={s.id ?? idx} className="p-3 bg-white/5 border border-white/6 rounded-lg text-sm">
                <div className="flex justify-between">
                  <div>
                    <div className="font-medium text-white">{s.event ?? s.type ?? "session"}</div>
                    <div className="text-xs text-white/60 mt-1">{JSON.stringify(s, null, 2).slice(0, 200)}</div>
                  </div>
                  <div className="text-xs text-white/50">{s.timestamp ? new Date(s.timestamp).toLocaleString() : "—"}</div>
                </div>
              </li>
            ))}
          </ul>
  ...
  const renderInsightsCard = () => (
    <Card className="bg-black/40 backdrop-blur-subtle border-white/10 shadow-2xl">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="text-2xl text-white font-semibold">UX Insights & Recommendations</CardTitle>
          ...
          <div className="flex items-center gap-2">
            {insights.length > 0 && (
              <Badge variant="outline" className="text-white/70 bg-white/5">
                {insights.length} {insights.length === 1 ? 'issue' : 'issues'} found
              </Badge>
            )}
            <Button
              size="sm"
              onClick={runAnalysis}
              disabled={isAnalyzing || (events.length === 0 && sessions.length === 0)}
              variant="outline"
                ...
                <div className="space-y-6">
                  ...
                  <Card className="bg-black/30 backdrop-blur border-white/10">
                    <CardHeader>
                      ...
                      <CardDescription className="text-white/60">Useful actions for debugging & onboarding</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="flex flex-col gap-2">
                        <Button onClick={fetchEvents} variant="outline" disabled={loadingEvents}>
                          {loadingEvents ? <RefreshCw className="h-4 w-4 animate-spin mr-2" /> : null}
                          Refresh Events
                        </Button>
                        <Button onClick={fetchSessions} variant="outline" disabled={loadingSessions}>
                          {loadingSessions ? <RefreshCw className="h-4 w-4 animate-spin mr-2" /> : null}
                          Refresh Sessions
                        </Button>
                        <Button onClick={copySnippet} variant="ghost">Copy Tracking Snippet</Button>
